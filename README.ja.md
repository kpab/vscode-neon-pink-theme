...
> **このテーマはまだ発展途上です。** ワークベンチの UI とシンタックスハイライトは一通り定義済みになりましたが、エディタの診断表示・差分エディタ・Git 装飾は VS Code 標準のダークテーマの色にフォールバックします。カバー範囲は[対応状況](#-対応状況)を、今後の予定は[ロードマップ](#-ロードマップ)をご覧ください。
...
`npm test` で [`scripts/check-contrast.js`](scripts/check-contrast.js) が走ります。シンタックスカラー × 3 種類のエディタ背景、および UI のラベル × それぞれの背景の計 299 組を測定し、1 つでも基準を下回れば失敗します。アルファ値を持つ色は合成してから測るので、スウォッチの見た目ではなく実際に目に届く色の値が出ます。除外しているのは空白文字マーカーとインデントガイドの 2 つだけで、これらは薄いままであるべきものです（除外理由も出力されます）。
