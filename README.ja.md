...
| Classic — 0.0.1 のパレットをバイト単位でそのまま凍結（[#24](https://github.com/kpab/vscode-neon-pink-theme/issues/24)） | ✅ 凍結済み |
| 上記すべてのスクリーンショット（実際の VS Code 画面） | ✅ README に掲載 |
| CI — push と PR ごとに構造・生成物の鮮度・コントラストを検証。`v*` タグで公開 | ✅ 自動化済み |

**アクセシビリティについて。** どのバリアントでも、すべてのテキストカラーが実際に描画される背景に対して WCAG AA（4.5:1）を満たしています。背景そのものだけでなく、選択範囲、現在行のハイライト、差分の両側も対象です。これらは背景の輝度を持ち上げるため、コントラスト比が約 1.3 分の 1 に落ちます。もっとも厳しいのは差分や選択範囲の中のキーワードで、Neon Pink Dark が 4.63:1、Soft が 4.58:1、Dimmed が 4.57:1。素の背景に対してなら同じキーワードが 6.4:1、コメント 6.8:1、関数 7.7:1、文字列 10.3:1、本文 18.0:1 です。**Classic** だけは意図的な例外です。この取り組みより前の 0.0.1 のパレットをそのまま再現しているため、上記の保証は適用されません。

`npm test` で [`scripts/check-contrast.js`](scripts/check-contrast.js) が走ります。シンタックスカラー × 12 種類の背景（エディタ、選択範囲、現在行、差分の追加/削除それぞれの行単位と語単位、インラインのマージコンフリクト 3 領域、3-way マージエディタ）、および UI のラベル × それぞれの背景で 1 テーマあたり 995 組。これを測定対象の 3 テーマ分、合計 2985 組を測定し、1 つでも基準を下回れば失敗します。Classic はスキップされ、理由も出力されます — AA まで引き上げてしまうと、残すべき見た目そのものが消えてしまうからです。アルファ値を持つ色は合成してから測るので、スウォッチの見た目ではなく実際に目に届く色の値が出ます。除外は空白文字マーカー・インデントガイド・差分エディタの斜線ハッチの 3 つだけで、いずれも薄いままであるべきものです（除外理由も出力されます）。

## 🎚️ バリアント

この拡張機能は 4 つのテーマを提供します。最初の 3 つは、色相も意味づけもルールも同じテーマを 3 段階の強さで用意したものです。切り替えても新しいテーマを覚え直す必要はなく、単に見やすさの選択になります。4 つ目はスナップショットで、0.0.1 として出荷されたパレットそのものです。
...
| **Neon Pink Dark Classic** | `#000000` | 100% | 0.0.1 の見た目が恋しい — VS Code 標準色の上に少しだけピンク |
...
**バリアントの作り方。** 手で保守するテーマは `themes/neon-pink-dark-color-theme.json` だけです。Soft と Dimmed の 2 つは [`scripts/build-themes.js`](scripts/build-themes.js) が生成してコミットします。インストール時に依存パッケージが要らないまま、3 つのパレットが別々にずれていくことも防げる形です（`npm test` が再生成して、ディスク上の内容と違えば失敗します）。変換は 3 段階です。
...
**Classic はバリアントではなくスナップショットです。** [#24](https://github.com/kpab/vscode-neon-pink-theme/issues/24) でのリクエストを受けて、0.0.1 当時のテーマをバイト単位でそのまま、独立した名前で収録したものです。定義しているのはワークベンチの 27 色だけで、残りは VS Code 標準の Dark+ に任せています — ソース管理の明るい青いコミットボタンもそのままです。それこそが「戻してほしい」と言われた控えめな見た目だからです。凍結は意図的なもので、コントラストの取り組みより前の姿のまま、`npm test` はスキップして理由を表示し、ベーステーマが変わっても Classic は変わりません。
