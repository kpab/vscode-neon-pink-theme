...
> **このテーマはまだ発展途上です。** ワークベンチの UI とシンタックスハイライトは一通り定義済みになりましたが、エディタの診断表示・差分エディタ・Git 装飾は VS Code 標準のダークテーマの色にフォールバックし、3 つの色が WCAG AA を下回っています。カバー範囲は[対応状況](#-対応状況)を、今後の予定は[ロードマップ](#-ロードマップ)をご覧ください。
