...
- A palette strip above the color table in both READMEs, drawn by `scripts/generate-palette.js` (`npm run palette`). The table lists ten hex codes and what each is for, which answers the question for people who read hex as color; the strip shows the colors at a size worth judging, with the code and the measured ratio under each. Like the banner, it reads every value out of the base theme and measures every ratio itself, so it cannot claim a number the theme no longer holds
...
### Fixed
- The contrast ratio listed for the types color in both README tables: `#FF9AD6` on black is 10.85:1, which is 10.8:1 at one decimal, not the 10.9:1 the table claimed. The palette strip measures its own ratios and surfaced the disagreement
